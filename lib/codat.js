...
export async function syncBills(companyId) {
  ...
  try {
    ...
    return [500, "Codat sync status operation error"];
  }
...
export async function getBillsCodat(companyId) {
  const companyBillUrl = `${codatPreUrl}/companies/${companyId}/data/bills?page=1&pageSize=100&orderBy=-issueDate`;
...
export async function getConnectionId(companyId) {
  const companyConnectionId = `${codatPreUrl}/companies/${companyId}/connections?page=1&pageSize=100`;
  try {
...
export async function payBillCodat(id, connectionId, companyId, accountId) {
  ...
  try {
    ...
    return [500, "Codat push operation key bill error"];
  }
